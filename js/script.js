  ...
  o = a.console && console.warn ? function (a) {
    ...
    if (a.length) {
      var c,
          ...
          o = b[s.ns],
          p = s.DPR;
      if (k = o.curSrc || b[G], l = o.curCan || j(b, k, a[0].set), l && l.set === a[0].set && (n = F && !b.complete && l.res - 0.1 > p, n || (l.cached = !0, l.res >= p && (h = l))), !h) for (a.sort(i), f = a.length, h = a[f - 1], d = 0; f > d; d++) {
        if (c = a[d], c.res >= p) {
...
$(document).ready(function () {
  ...
  var linksWithPopup = document.querySelectorAll(".nav__link-item[aria-haspopup='true']");
  var sublinksWithPopupSelector = ".nav__sub-link-item[aria-haspopup='true']";

  function closeSubmenus(links) {
    links.forEach(function (link) {
      link.setAttribute("aria-expanded", "false");

      closeSubmenus(link.nextElementSibling.querySelectorAll(sublinksWithPopupSelector));
    });
  }

  function popupHandler(link) {
    link.addEventListener("click", function () {
      var alreadyExpanded = this.getAttribute("aria-expanded") === "true";
  ...
  function attachMenuExpandHandlers(links) {
    links.forEach(function (link) {
      ...
      attachMenuExpandHandlers(link.nextElementSibling.querySelectorAll(sublinksWithPopupSelector));
    });
  }
  ...
  //   const sublinksWithPopup = link.nextElementSibling.querySelectorAll(
  //     ".nav__sub-link-item[aria-haspopup='true']"
  //   );

  //   sublinksWithPopup.forEach(link => {
  //     popupHandler(link);
  //   });
  // });
});